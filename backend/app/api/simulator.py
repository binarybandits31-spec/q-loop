...
async def simulator_run(
    ...
    Equivalent to /api/circuits/run but requests statevector
    when possible.
    """
    ...
    result = backend.execute(
        circuit_dict,
        shots=body.shots,
        include_statevector=include_sv,
    )
...
async def list_backends():
    ...
    backends_info = [
        ...
        },
        {
            ...
            "name": "PennyLane",
            "description": (
                "Differentiable quantum computing simulator."
            ),
            "is_operational": False,
            "is_default": False,
        },
        {
            ...
            "name": "Cirq",
            "description": (
                "Google's quantum circuit simulator."
            ),
            "is_operational": False,
            "is_default": False,
        },
    ...
    try:
        ...
        backends_info[0]["note"] = (
            "Install qiskit-aer to enable."
        )

    except Exception as e:
        backends_info[0]["note"] = str(e)

    # Verify PennyLane availability
    try:
        from ..services.quantum.pennylane_adapter import PennyLaneBackend

        backends_info[1]["is_operational"] = (
            PennyLaneBackend().is_available
        )

    except Exception as e:
        backends_info[1]["note"] = str(e)

    # Verify Cirq availability
    try:
        from ..services.quantum.cirq_adapter import CirqBackend

        backends_info[2]["is_operational"] = (
            CirqBackend().is_available
        )
