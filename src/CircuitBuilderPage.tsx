...
export default function CircuitBuilderPage() {
  ...
  const handleSimulate = async () => {
    ...
    // Exclude measurement gates when calculating the pre-measurement state vector.
    const stateVectorGates = circuitGates.filter((g) => g.type !== 'M')
    ...
    try {
      ...
      if (!res.success || res.error) {
        ...
        const local = simulateCircuit(stateVectorGates, numQubits, 1024)
        setSimResult(local)
      } else {
        ...
        // Build the displayed state vector before measurement collapse.
        const local = simulateCircuit(stateVectorGates, numQubits, 0)
        setSimResult({
