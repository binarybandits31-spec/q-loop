...
import { Terminal, Play, Zap, BarChart3, Copy, Check, Code2, Loader2, AlertCircle } from 'lucide-react';
...
// Map of DSL gate names -> GateType. Controlled gates are recognized by
// their leading C and handled specially below.
...
function parseCircuit(code: string): ParseResult {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    let line = (commentIdx >= 0 ? lines[i].slice(0, commentIdx) : lines[i]).trim();
    if (!line) continue;

    // opening: circuit Name {
    if (!inCircuit) {
      const m = line.match(/^circuit\s+(\w+)\s*\{?$/i);
      if (!m) return { error: `Expected "circuit Name {" at line ${lineNum}`, line: lineNum };
      name = m[1];
      inCircuit = true;
      // allow "circuit Name {" on one line or "circuit Name" + "{" on next
      if (line.endsWith('{')) foundBrace = true;
      continue;
    }
    ...
    if (!foundBrace) {
      ...
      return { error: `Expected "{" after circuit declaration at line ${lineNum}`, line: lineNum };
    }

    // closing brace
    if (line === '}') {
      if (numQubits === 0) return { error: 'No "qubits:" declaration found', line: lineNum };
      return { name, numQubits, gates };
    }
    ...
    if (qm) {
      ...
      if (numQubits < 1) return { error: `qubits must be >= 1 at line ${lineNum}`, line: lineNum };
      if (numQubits > 8) return { error: `qubits must be <= 8 at line ${lineNum}`, line: lineNum };
      continue;
    }

    if (numQubits === 0) {
      return { error: `Gate before "qubits:" declaration at line ${lineNum}`, line: lineNum };
    }

    // gate line: NAME(args)? q[a], q[b], q[c]
    // Extract optional parameter list first.
    const gateMatch = line.match(/^([A-Za-z]+)\s*(?:\(\s*([^)]*)\s*\))?\s*(.*)$/);
    if (!gateMatch) return { error: `Invalid syntax at line ${lineNum}: "${line}"`, line: lineNum };
    ...
    if (!gateType) return { error: `Unknown gate "${rawName}" at line ${lineNum}`, line: lineNum };

    // parse operands: q[0], q[1], ...
    const operands = operandsStr
      ? operandsStr.split(',').map((s) => s.trim()).filter(Boolean)
      : [];

    const qubitIndices: number[] = [];
    for (const op of operands) {
      const qm2 = op.match(/^q\[(\d+)\]$/i);
      if (!qm2) return { error: `Invalid qubit reference "${op}" at line ${lineNum}`, line: lineNum };
      const idx = parseInt(qm2[1], 10);
      if (idx < 0 || idx >= numQubits) {
        return { error: `Qubit index ${idx} out of range (0..${numQubits - 1}) at line ${lineNum}`, line: lineNum };
      }
    ...
    if (paramStr !== undefined && paramStr !== '') {
      ...
      if (Number.isNaN(val)) return { error: `Invalid parameter "${paramStr}" at line ${lineNum}`, line: lineNum };
      parameter = val;
    }

    const id = `${gateType}-${i}-${Math.random().toString(36).slice(2, 7)}`;

    // build the CircuitGate according to gate type
    if (gateType === 'M') {
      if (qubitIndices.length !== 1) return { error: `measure expects 1 qubit at line ${lineNum}`, line: lineNum };
      gates.push({ id, type: 'M', qubit: qubitIndices[0] });
    } else if (gateType === 'SWAP') {
      if (qubitIndices.length !== 2) return { error: `SWAP expects 2 qubits at line ${lineNum}`, line: lineNum };
      gates.push({ id, type: 'SWAP', qubit: qubitIndices[0], targetQubit: qubitIndices[1] });
    } else if (gateType === 'TOFFOLI') {
      if (qubitIndices.length !== 3) return { error: `Toffoli expects 3 qubits at line ${lineNum}`, line: lineNum };
      gates.push({
        id,
        type: 'TOFFOLI',
    ...
    } else if (gateType === 'CX' || gateType === 'CY' || gateType === 'CZ' || gateType === 'CH') {
      if (qubitIndices.length !== 2) return { error: `${rawName} expects 2 qubits at line ${lineNum}`, line: lineNum };
      gates.push({
        id,
        type: gateType,
      ...
      // single-qubit gate
      if (qubitIndices.length !== 1) return { error: `${rawName} expects 1 qubit at line ${lineNum}`, line: lineNum };
      if (PARAM_GATES.has(gateType) && parameter === undefined) {
        return { error: `${rawName} requires a parameter, e.g. ${rawName}(3.14) at line ${lineNum}`, line: lineNum };
      }
      gates.push({ id, type: gateType, qubit: qubitIndices[0], parameter });
    }
  }

  if (!foundBrace) return { error: 'Missing opening brace "{"', line: lines.length };
  return { error: 'Missing closing brace "}"', line: lines.length };
}
...
export default function SimulatorPage() {
  ...
  const formattedState = result ? formatStateVector(result.stateVector) : [];
  const maxProb = Math.max(...formattedState.map((s) => s.probability), 0.0001);
  const maxCount = result && result.histogram.length > 0 ? Math.max(...result.histogram.map((h) => h.count)) : 1;
        ...
        <header className="mb-8 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <div className="mb-2 flex items-center gap-2 text-cyan-400">
              ...
              <span className="text-xs font-mono uppercase tracking-[0.2em]">Q-Loop / Simulator</span>
            </div>
          ...
          {Object.keys(EXAMPLES).map((key) => (
            <Button
              ...
              className={cn(
                ...
                code === EXAMPLES[key] && 'border-cyan-400/60 bg-cyan-500/15 text-cyan-50'
              )}
            >
              {key}
            </Button>
              ...
              <Card className="border-rose-500/40 bg-rose-950/40 backdrop-blur-xl">
                <CardContent className="p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-sm font-mono font-semibold text-rose-300">Parse Error</p>
                      <p className="mt-1 font-mono text-xs text-rose-200/80">{parseError}</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className="border-amber-500/40 bg-amber-950/40 backdrop-blur-xl">
                <CardContent className="p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-sm font-mono font-semibold text-amber-300">Backend Error</p>
                      <p className="mt-1 font-mono text-xs text-amber-200/80">{backendError}</p>
                ...
                {!result || formattedState.length === 0 ? (
                  ...
                  <div className="space-y-1.5">
                    ...
                    {formattedState.map((s) => (
                      ...
                      >
                        <span className="font-mono text-xs text-cyan-200">|{s.basis}⟩</span>
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-mono text-[11px] text-cyan-100/70">{s.amplitude}</span>
                        </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    {result.histogram.map((h) => (
                      <div
                        ...
                        className="grid grid-cols-[5rem_1fr_5rem] items-center gap-2 rounded-md border border-cyan-500/10 bg-slate-900/40 px-2 py-1.5"
                      >
                        <span className="font-mono text-xs text-cyan-200">|{h.state}⟩</span>
                        <div className="flex items-center gap-2">
                          <div className="h-2 min-w-[3rem] flex-1 overflow-hidden rounded-full bg-slate-800/60">
                            <div
                          ...
                          <span className="font-mono text-[11px] text-cyan-100/70">{h.count}</span>
                        </div>
                        <span className="text-right font-mono text-[11px] text-cyan-300">
